...
public:
    fltkview(int x, int y, int w, int h, const char *l) : Fl_Gl_Window(x, y, w, h, l),
                                                          ...
                                                          vertex_shader(0), pixel_shader(0), program(0), texture(0), pick_vert(-1), tex_(0)
    {
        lastmouse[0] = 0.0f;
        lastmouse[1] = 0.0f;
        extract[0] = 0.0f;
    ...
    void make_texture()
    {
        ...
        BYTE *pixels = FreeImage_GetBits(im);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, FreeImage_GetWidth(im), FreeImage_GetHeight(im), 0,
    ...
    void draw()
    {
        ...
        if(ar)
        {
            ...
            std::vector<vertex> v;
            std::vector<vec3u> f;
            ar->make_mesh(v, f, vec2f(0.0, 1.0), vec2f(-2.5, 2.5), 0.01);
